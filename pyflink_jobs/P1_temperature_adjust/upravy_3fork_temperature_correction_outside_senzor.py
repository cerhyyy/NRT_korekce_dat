...
wm = WatermarkStrategy.for_bounded_out_of_orderness(Duration.of_minutes(60)) \
    .with_timestamp_assigner(EventTimestampAssigner()).with_idleness(Duration.of_seconds(3))
wm2 = WatermarkStrategy.for_bounded_out_of_orderness(Duration.of_minutes(60)) \
    .with_timestamp_assigner(EventTimestampAssigner()).with_idleness(Duration.of_seconds(3))
...
class WaitAndJoin(CoProcessFunction):
    ...
    def process_element1(self, device, ctx):
        # Check if the event is late according to the watermark
        if ctx.timestamp() < ctx.timer_service().current_watermark():
            print(f"Dropping late event: {device[0]}")
            return
    ...
    def process_element2(self, meteo, ctx):
        # Check if the event is late according to the watermark
        if ctx.timestamp() < ctx.timer_service().current_watermark():
            print(f"Dropping late meteo event: {meteo[0]}")
            return

        meteo_time_at_arrival_ms = meteo[4]
        meteo_time_at_arrival_ms = time.perf_counter() - meteo_time_at_arrival_ms
